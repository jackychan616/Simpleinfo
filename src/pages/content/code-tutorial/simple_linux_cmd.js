...
function ConContent() {
  return (
    <>
      <Container>
        <ConTitle order={1}>Linux幾個必學指令(ls cd cat)</ConTitle>
        ...
        <p id="introduction-cotent">
          Linux 是一種開放和免費的源代碼，可以替代window 作系統用。另外，Linux
          的操作系統提供了不同工具和支持多種語言令開發者更自由選擇。
        </p>

        <ConTitle order={2}>ls</ConTitle>

        <ConTitle order={3}>用途</ConTitle>
        <p>顯示當前路徑文件</p>

        <ConTitle>其他指令</ConTitle>
        <code>
          <pre> ls [路徑] 顯示其他路徑文件 </pre>{' '}
        </code>
        <code>
          <pre> -d */ 只顯示檔案 </pre>{' '}
        </code>
        <code>
          <pre> -R 顯示當前路徑中檔案子檔案 </pre>{' '}
        </code>
        <code>
          <pre> -l 顯示檔案資訊 </pre>{' '}
        </code>
        ...
        <ConTitle order={3}>cd</ConTitle>
        <ConTitle order={3}>用途</ConTitle>
        <p>切換路徑</p>
        <ConTitle order={3}>示範</ConTitle>
        <Image
          src="/img/img2.png"
          id="img2"
          width="500"
